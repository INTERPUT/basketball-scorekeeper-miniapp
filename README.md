...
这个项目把技术台流程收进手机：比赛开始前设置四位房间码，比赛中用自然语言快速记录事件，系统自动转成得分、犯规、暂停和提醒，观众用房间码实时看比分，比赛结束后双方队长在设备上签字，系统生成正式计分表、PDF 报告和结构化归档。

## 为什么值得看

- 更轻：把校园比赛常见的“两人计分互校 + 一人计时翻分”缩减到一名技术员。
- 更快：技术员只要说清楚或输入场上事件，确认后直接入账。
- 更稳：比分、犯规、暂停、节次和提醒都由事件流回放生成，个人和球队犯规到临界点会提醒。
- 更透明：观众和球员用四位房间码进入同一场比赛，只读查看实时信息。
- 更完整：赛后签字、正式计分表、PDF 报告、结构化数据一次生成，包含球队得分和球员个人得分，方便复盘和分享。

## 项目亮点

### 从纸质技术台到手机技术台

技术员不再一边听口令、一边翻表格、一边手算比分。小程序把“自然语言录入、确认、统计、提醒、同步、导出”串成一条完整流程，降低懂球但不熟悉专业计分表的临时技术员门槛。

### 文本和语音走同一条确认链路

“白队 7 号两分命中”“蓝队暂停”“比赛结束”这类口语化输入会先变成待确认事件，技术员确认后才正式生效。它解决的是现有按钮式计分小程序仍要找按钮、切模块的问题；语音只是更快的输入方式，不会绕过人工确认。
...
比赛结束后，双方队长完成电子签字，系统自动生成归档版本。正式计分表 HTML、PDF 报告和结构化 JSON 都绑定同一份事件流，PDF 报告额外给出球员个人得分，方便主办方复核，也给参赛者留下可分享的数据结果。
