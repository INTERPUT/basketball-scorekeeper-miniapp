import type { DraftEvent, Match, MatchSnapshot, ParseEventResult, Team } from "./models";
...
function findTeam(match: Match, text: string): Team | undefined {
  return match.teams.find((team) => text.includes(team.name));
}

function findPlayerByNumber(team: Team, text: string) {
...
function createDraft(
  ...
  sourceText: string,
  input: Omit<DraftEvent, "draftEventId" | "period" | "gameClockSeconds" | "sourceText">
): DraftEvent {
  return {
...
function getClockStartPeriod(snapshot: MatchSnapshot): number {
  return snapshot.clock.status === "period_ended" ? snapshot.currentPeriod + 1 : snapshot.currentPeriod;
}

function parseClockText(snapshot: MatchSnapshot, rawText: string, compactText: string): ParseEventResult | undefined {
  if (compactText === "开始计时") {
...
export function parseEventText(match: Match, snapshot: MatchSnapshot, rawText: string): ParseEventResult {
