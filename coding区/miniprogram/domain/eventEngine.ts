...
interface TeamAccumulator {
  ...
  timeoutsByBucket: Record<string, number>;
  players: Record<string, { points: number; fouls: number }>;
}
...
interface ReplayState {
  ...
  teams: Record<string, TeamAccumulator>;
  alerts: Alert[];
}

function createAlertId(type: string, eventId: string): string {
  return `${type}_${eventId}`;
}

function getTimeoutBucket(period: number): string {
  if (period <= 2) {
    return "first_half";
  }
...
function getTimeoutLimit(period: number): number {
  if (period <= 2) {
    return 2;
  }
...
function getScoreValue(event: ConfirmedEvent): number {
...
function ensurePeriodScore(state: ReplayState, period: number): [number, number] {
...
function getTeamIndex(match: Match, teamId: string): 0 | 1 {
...
function assertPlayerExists(state: ReplayState, teamId: string, playerId: string): void {
  if (!state.teams[teamId]?.players[playerId]) {
    throw new Error("事件引用了不存在的球员。");
  }
}

function ensureLastMinuteAlert(state: ReplayState, event: ConfirmedEvent, remainingSeconds: number): void {
  if (remainingSeconds > 60) {
    return;
  }

  const alreadyTriggered = state.alerts.some(
    (alert) => alert.type === "last_minute_alert" && alert.period === event.period
  );
...
function createInitialState(match: Match): ReplayState {
  return {
    status: "draft",
    currentPeriod: 1,
    clock: {
...
function assertClockActionAllowed(state: ReplayState, event: ConfirmedEvent): void {
  switch (event.eventType) {
    case "clock_start":
...
export function recomputeSnapshot(match: Match, room: Room, events: ConfirmedEvent[]): MatchSnapshot {
