...
function assertOperator(match) {
  ...
  if (openid && match.operatorId && openid !== match.operatorId) {
    throw new Error("只有创建比赛的技术员可以确认事件。");
  }
}
...
function assertPlayerExists(state, teamId, playerId) {
  if (!state.teams[teamId] || !state.teams[teamId].players[playerId]) {
...
function ensureLastMinuteAlert(state, event, remainingSeconds) {
  ...
  if (!exists) {
    state.alerts.push({
      alertId: `last_minute_alert_${event.eventId}`,
    ...
    switch (event.eventType) {
      ...
      case "personal_foul": {
        ...
        if (playerState.fouls === 5) {
          state.alerts.push({
            alertId: `player_foul_out_alert_${event.eventId}`,
        ...
        if (teamState.currentPeriodFouls >= 5) {
          state.alerts.push({
            alertId: `team_bonus_alert_${event.eventId}`,
      ...
      case "team_timeout": {
        ...
        const nextUsage = (teamState.timeoutsByBucket[bucket] || 0) + 1;
  ...
  return {
    ...
    teams: match.teams.map((team) => {
      ...
      return {
        ...
        timeoutsRemaining: getTimeoutLimit(state.currentPeriod) - (teamState.timeoutsByBucket[bucket] || 0)
      };
    }),
...
exports.main = async (event) => {
  ...
  const match = await db.collection("matches").doc(room.matchId).get().then((result) => result.data);
  ...
  const existingEvents = await db
    ...
    .limit(1000)
    .get()
    .then((result) => result.data.map((item) => item.event));
  const nextEvent = buildConfirmedEvent(event.draft);
  const nextEvents = [...existingEvents, nextEvent];
  const snapshot = recomputeSnapshot(match, room, nextEvents);

  await Promise.all([
    db.collection("events").doc(nextEvent.eventId).set({
      data: {
    ...
    db.collection("snapshots").doc(room.matchId).set({
      data: snapshot
    }),
